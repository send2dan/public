...
if (!is.na(params$carrier) && params$carrier != "NA") {
  flights <- flights[flights$carrier == params$carrier, ]
}
if (nrow(flights) == 0) {
  stop("Invalid selection. Did you misspell `carrier`?")
}
...
Creating parameterised reports (i.e., documents with outputs—text, plots, tables, etc—that change based on the values of the parameters that you select) is a really common task for data analysts, and it’s a task that can be made much easier with {quarto}.
...
## Rendering parameterised reports using Quarto (1)
...
## Rendering parameterised reports using Quarto (2)
...
## Rendering parameterised reports using Quarto (3)
...
`r flights |> filter(is.na(dep_delay)) |> count()` flight departures were cancelled.
...
The average delay was `r planes_with_delays |> filter(!is.na(avg_delay)) |> summarise(mean = mean(avg_delay))` minutes.
...
The following plot shows the average departure delay (by destination  airport), concentrating on destinations taking more than a 2000 flights in `r unique(flights$year)` from `r unique(flights$origin)`.
...
Note extremely [useful resource](https://meghan.rbind.io/blog/quarto-pdfs/) published by [Meghan Hall](twitter.com/meghanmhall)
